...
function createAttempt(mode) {
  ...
  if (mode === 'mistakes') {
    ...
    if (!selected.length) throw new Error('Aucune question incorrecte n’est encore enregistrée pour révision.');
...
export async function initExam() {
  try {
    $('#data-status').textContent = 'Chargement de la banque de questions…'; bank = await loadQuestionBank(); $('#data-status').textContent = '● Données chargées : 500 questions';
    ...
    if (questions.length !== attempt.questionIds.length) throw new Error('Certaines questions de la tentative n’ont pas pu être restaurées.');
  ...
  } catch (error) { $('#exam-main').innerHTML = `<div class="notice error" role="alert">${esc(error.message)}</div><p><a class="btn btn-secondary" href="./index.html">Retour à l’accueil</a></p>`; }
}
...
function renderTimer() { $('#timer').textContent = formatTime(attempt.remainingSeconds); $('#timer').classList.toggle('status-bad', attempt.remainingSeconds <= 600); if(attempt.remainingSeconds===600)announce('Il reste dix minutes.'); }
...
function render() {
  const q = currentQuestion(); const idx = attempt.current; $('#question-number').textContent = `Question ${idx + 1} sur ${questions.length}`; $('#question-type').textContent = typeLabel(q.type);
  ...
  const caseBox = $('#case-box'); if (q.case_id) { caseBox.hidden = false; caseBox.innerHTML = `<details open><summary><strong>${esc(q.case_title)}</strong> — afficher ou masquer le scénario</summary><p>${esc(q.case_scenario)}</p></details>`; } else caseBox.hidden = true;
  renderQuestion(q, attempt.answers[q.qid], { onChoice: choice, onReorder: reorder, onMatch: match });
  $('#flag-button').textContent = attempt.flags.includes(q.qid) ? '★ Retirer le marquage' : '☆ Marquer pour révision';
  $('#previous').disabled = idx === 0 || lockedByBreak(idx - 1); $('#next').textContent = idx === questions.length - 1 ? 'Réviser et terminer' : 'Suivante'; refreshNavigation(); announce(`Question ${idx + 1} sur ${questions.length}`);
}
...
function buildNavigation(){const grid=$('#question-grid');grid.innerHTML=questions.map((_,i)=>`<button class="qnav" type="button" data-index="${i}" aria-label="Aller à la question ${i+1}">${i+1}</button>`).join('');grid.addEventListener('click',e=>{const b=e.target.closest('[data-index]');if(!b)return;const target=Number(b.dataset.index);if(!lockedByBreak(target)){attempt.current=target;render()}})}
...
function confirmSubmit(){const unanswered=questions.filter(q=>!isAnswered(q,attempt.answers[q.qid])).length;$('#confirm-text').textContent=unanswered?`${unanswered} question(s) sont sans réponse. Vous pouvez y revenir ou soumettre l’examen maintenant.`:'Vous avez répondu à toutes les questions. Souhaitez-vous soumettre l’examen ?';$('#submit-dialog').showModal()}
