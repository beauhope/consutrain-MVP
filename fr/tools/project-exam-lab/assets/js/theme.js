...
export function applyTheme(theme) {
  ...
  document.querySelectorAll('[data-theme-toggle]').forEach(btn => { const label=resolved === 'dark' ? '☀ Mode clair' : '☾ Mode sombre';btn.textContent=label;btn.setAttribute('aria-label',`Passer au ${label.replace(/^[^ ]+ /,'').toLowerCase()}`); });
}
