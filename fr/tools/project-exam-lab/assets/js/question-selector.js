...
export function selectQuestions(questions, options = {}) {
  ...
  if (eligible.length < count) throw new Error(`Les filtres actuels proposent seulement ${eligible.length} question(s), alors que ${count} sont demandées.`);
  ...
  if (count === 180 && !Object.values(options.filters || {}).some(values => values?.length)) {
    ...
    if (selected.length !== count) throw new Error('La banque ne contient pas la répartition nécessaire pour construire l’examen complet.');
  ...
  if (selected.length !== count || new Set(selected.map(q => q.qid)).size !== count) throw new Error('Impossible de constituer un examen unique avec la répartition demandée.');
  return fisherYates(selected, random);
}
