(function () {
  ...
  const fallback = (kind, message) => kind === 'header'
    ? `<header class="site-header includes-fallback"><nav class="nav" aria-label="Navigation principale"><a class="brand-text" href="${new URL('index.html', appRoot)}">ConsuTrain — Simulateur de gestion de projet</a><span>${message}</span></nav></header>`
    : `<footer class="site-footer includes-fallback"><div class="footer-inner"><p>© ${new Date().getFullYear()} ConsuTrain — Tous droits réservés.</p><p>Développé par Youssef Joul.</p><p>${message}</p></div></footer>`;
  ...
  async function inject(id, file, kind) {
    ...
    try {
      ...
      console.error(`Échec du chargement de ${file} depuis ${appRoot.href}`, error);
      host.innerHTML = fallback(kind, 'Certains éléments de l’interface n’ont pas pu être chargés. Vous pouvez revenir à l’accueil.');
    }
  }
  function activate() {
    ...
    toggle?.addEventListener('click', () => { const open=toggle.getAttribute('aria-expanded')!=='true'; toggle.setAttribute('aria-expanded',String(open)); toggle.setAttribute('aria-label',open?'Fermer le menu':'Ouvrir le menu');links.classList.toggle('is-open',open);document.body.classList.toggle('nav-open',open); });
