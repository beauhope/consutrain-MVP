...
export function validateQuestionBank(bank, expectedCount = CONFIG.expectedQuestions) {
  ...
  if (!bank || !Array.isArray(bank.questions)) return { valid: false, errors: ['Le champ questions est absent ou n’est pas un tableau.'], warnings, stats: null };
  const questions = bank.questions;
  if (questions.length !== expectedCount) errors.push(`${expectedCount} questions attendues ; ${questions.length} trouvées.`);
  const ids = questions.map(q => q?.qid).filter(Boolean);
  const duplicates = [...new Set(ids.filter((id, i) => ids.indexOf(id) !== i))];
  if (duplicates.length) errors.push(`Identifiants qid en double : ${duplicates.join(', ')}`);
  const missingExplanations = [], missingAnswers = [], badCaseRefs = [], invalidQuestions = [];
  const caseIds = new Set((bank.cases || []).map(c => c.id));
  questions.forEach((q, index) => {
    const tag = q?.qid || `question ${index + 1}`;
    const required = ['qid', 'prompt', 'type', 'domain', 'approach', 'difficulty'];
    if (required.some(k => !text(q?.[k]))) invalidQuestions.push(`${tag} : champs obligatoires manquants`);
    ...
    } else invalidQuestions.push(`${tag} : type non pris en charge (${q?.type ?? 'vide'})`);
    if (!answerValid) missingAnswers.push(tag);
  });
  if (invalidQuestions.length) errors.push(`${invalidQuestions.length} question(s) ont une structure incorrecte.`);
  if (missingAnswers.length) errors.push(`${missingAnswers.length} question(s) n’ont pas de réponse correcte valide.`);
  if (missingExplanations.length) warnings.push(`${missingExplanations.length} question(s) n’ont pas d’explication.`);
  if (badCaseRefs.length) errors.push(`${badCaseRefs.length} référence(s) d’étude de cas sont invalides.`);
...
export async function loadQuestionBank(url = CONFIG.dataUrl) {
  ...
  try {
    ...
    console.error('Impossible de charger la banque de questions :', error instanceof Error ? error.message : error);
    throw new Error('Impossible de charger la banque de questions. Vérifiez la connexion puis rechargez la page. Utilisez un serveur local ou GitHub Pages.');
  }
}
