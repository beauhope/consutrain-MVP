...
- لا تضع مفاتيح أو رموز وصول أو بيانات دخول أو عناوين Webhook داخلية أو معلومات شخصية حساسة في الوثائق أو الشيفرة العامة. endpoint التحقق العام الموثق في `CERTIFICATE-SYSTEM.md` ليس سرًا تشغيليًا داخليًا.
...
## 9. منظومة الشهادات والتحقق

- المرجع الإنتاجي الكامل هو [`CERTIFICATE-SYSTEM.md`](CERTIFICATE-SYSTEM.md)، وحالته منذ 2026-09-05 هي **COMPLETED / PRODUCTION VERIFIED**.
- لا تستخدم `ConsuTrain Certificate Queue Processor` القديم؛ الإصدار `ConsuTrain Certificate Queue Processor v2` هو المعتمد.
- لا تغيّر صيغة `verificationCode` دون Migration واضحة، ولا تغيّر أعمدة `Certificate_Verifications` دون تحديث `Certificate Verification Lookup`.
- لا تحذف `verificationCode` أو `verificationUrl` من مدخلات `Unified Certificate Delivery`، ويجب أن يدعم أي قالب شهادة جديد الحقلين.
- يجب أن تدعم أي لغة جديدة صفحة التحقق ورسائل API وواجهة النتيجة.
- لا تعرض صفحة التحقق بيانات داخلية إضافية أو `submissionId`، وتعامل مع `Certificate_Verifications` كسجل التحقق الرسمي.
- اختبر مسار PDF → Verification Page بعد أي تعديل جوهري على منظومة الشهادات.
