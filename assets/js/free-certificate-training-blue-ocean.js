(function () {
  ...
  async function retryPendingSubmissions() {
    ...
    for (const entry of matching) {
      try {
        const result = await submitPayload(entry.payload);
        if (isTerminalBackendResult(result)) lastResult = result;
        else failed.push(entry);
      } catch (error) { failed.push(entry); }
    }
    ...
    refreshPendingUI();
    if (failed.length) setStatus("تعذرت إعادة إرسال بعض الطلبات أو تأكيد نتيجتها. ستبقى محفوظة مؤقتًا حتى انتهاء المدة أو حذفها.", "error");
    else showBackendResult(lastResult, "تم إرسال الطلبات المعلقة بنجاح.");
  }
