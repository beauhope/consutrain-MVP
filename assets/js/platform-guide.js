(() => {
  ...
  const resolveUrl = (value) => {
    ...
    return `${root}/${value}`.replace(/\/\.\//g, "./").replace(/([^:])\/\/{2,}/g, "$1/");
  };
  ...
  const renderCard = (item, sectionId) => {
    ...
    const link = create("a", "platform-guide-card__link", item.label || "فتح الرابط");
  ...
  const renderSection = (section, index) => {
    ...
    const count = create(
      ...
      `${section.items.length} ${section.items.length > 10 ? "عنصرًا" : "عناصر"}`
    );
  ...
  const updateDate = (dateValue) => {
    ...
    updatedAt.textContent = new Intl.DateTimeFormat("ar-MA", {
      year: "numeric",
      month: "long",
      day: "numeric"
    }).format(date);
  ...
  const applySearch = () => {
    ...
    document.querySelectorAll(".platform-guide-section").forEach((section) => {
      const visibleCards = section.querySelectorAll(".platform-guide-card:not([hidden])").length;
      section.hidden = Boolean(query) && visibleCards === 0;
    });

    if (!resultsText) return;
    resultsText.textContent = query
      ? `تم العثور على ${visible} نتيجة من أصل ${totalItems}.`
      : `يعرض الدليل ${totalItems} عنصرًا موزعة على ${guideData.sections.length} أقسام.`;
  };
  ...
  const renderError = () => {
    ...
    card.appendChild(create("h2", "", "تعذر تحميل محتويات الدليل"));
    card.appendChild(
      create(
        "p",
        "",
    ...
    const link = create("a", "btn btn-primary", "تحميل الدليل PDF");
    link.href = resolveUrl("resources/downloads/profile/ConsuTrain_Profile_and_Services_AR.pdf");
  ...
  const init = async () => {
    try {
      const response = await fetch(
        resolveUrl("assets/data/platform-guide.json"),
        { cache: "no-store" }
      );
